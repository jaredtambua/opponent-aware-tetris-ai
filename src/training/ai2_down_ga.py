...
# ai that always uses "danger" weights
class DangerOnlyAI2(TetrisAI2):
    def __init__(self, genome, ply=6, well_col=9, gamma=0.95, depth_beam_width=14):
        ...
        # extend genome with unused keys required by base ai
        full_genome = dict(genome)
        full_genome["FILL_WELL"] = 0.0
        full_genome["WELL_HEIGHT"] = 0.0

        self.SAFE = copy.deepcopy(full_genome)
        self.W_DANGER = copy.deepcopy(full_genome)
        self.W = self.W_DANGER
        self.in_danger = True

    # always force danger mode before choosing an action
...
def simulate_genome(genome, seed, ply=6):
    ...
    genome = dict(genome)
    genome["ANY_CLEAR_PENALTY"] = -abs(genome["ANY_CLEAR_PENALTY"])

    board = Board(rows=BOARD_ROWS, cols=BOARD_COLS)
    game = Game(board)
    ai = DangerOnlyAI2(genome=genome, ply=ply)
...
def mutate_gene(value):
    ...
    return clamp_nonnegative(value * math.exp(random.gauss(0.0, 0.8)))
...
def run_ga(
    ...
    for generation in range(1, generations + 1):
        ...
        print(
            f"generation {generation}: best pieces placed = {gen_best_score:.2f} "
    ...
    print("\nbest overall average pieces placed:", f"{best_overall_score:.2f}")
